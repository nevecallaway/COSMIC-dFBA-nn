...
DECOUPLED scheme: the initial pool stays at realistic DMEM (unscaled) and only the
perfusion FEED is enriched by scale s. Each AA's concentration is then affine in s:

    C_i(t; s) = R_i(t) + s * P_feed_i(t)

where P_feed_i(t) = cin0 (1 - e^{-Ft}) is the feed part that scales, and
R_i(t) = c0 e^{-Ft} + Q_i(t) is everything that does not (the initial-pool
relaxation plus the negative consumption Q_i, driven by v * X, unscaled). For
non-negativity C_i(t) >= FLOOR we need

    s >= (FLOOR - R_i(t)) / P_feed_i(t)   for all t with feed present,

so the minimum per-AA FEED scale is the max of that ratio over all reactors and
time, at the worst DoE for feed (AAs = -1, lowest feed). A margin adds headroom.
Q_i is recovered as the generator's trajectory minus the scale-1 media part.
Sizing only the feed lets a consumed AA deplete toward FLOOR (restoring the
data_2 dynamics) instead of being pinned flat when initial == feed.
...
def main():
    ...
    # Run each reactor once at DMEM initial + DMEM feed (scale 1), AAs = -1, to
    # recover the consumption part Q_i. Q_i is independent of the AA media scale:
    # uptake v*X depends on the cell-density trajectory, not on AA levels.
    ...
    for r in reactor_ids:
        ...
        for k, j in enumerate(aas):
            c0    = DMEM_AA[k]                   # DMEM initial (NOT scaled -- decoupled)
            cin0  = c0 * feed_mult              # feed at AAs = -1, scale 1
            base  = c0 * eF + cin0 * (1.0 - eF)  # media part at scale 1 (what traj used)
            Q     = traj[:, j] - base            # consumption part (scale-independent)
            # DECOUPLED model: only the feed scales. At feed-scale s,
            #   C_i(t) = [c0*eF + Q]  +  s * [cin0*(1-eF)]  = R + s*P_feed
            # so non-negativity C_i >= FLOOR needs s >= (FLOOR - R) / P_feed.
            R      = c0 * eF + Q                 # unscaled part (initial relax + uptake)
            P_feed = cin0 * (1.0 - eF)           # feed part that scales with s
            with np.errstate(divide='ignore', invalid='ignore'):
                ratio = np.where(P_feed > 1e-9, (FLOOR - R) / P_feed, 0.0)
            req[k] = max(req[k], float(ratio.max()))
