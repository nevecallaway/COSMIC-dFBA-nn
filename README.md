...
A physics-informed (PINN) variant, with concentrations predicted directly and the
ODE used as a soft training penalty (solver-free at inference), was also explored
and is archived under [`nn/scripts_past/pinn_baseline.py`](nn/scripts_past/pinn_baseline.py).
